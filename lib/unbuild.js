...
const startPattern = /USE\(["']\.\/[\w/]+["'],\s*function\s*\((|\w+|\w+,\s*exports)?\)\s*\{/g;
const getFilenamePattern = /USE\s*\(['"](.+)['"],\s*function\s*\(\s*(|\w+|\w+,\s*exports)\s*\)\s*\{/;
...
const undent = (codeLines, n) => {
	...
	return codeLines
		...
		.replace(regex, '\n')
		.trim()
		.replace(/(const|let|var)\s+([{\s\w,}]+)\s+=\s+(USE)\s*\((['"].+['"])\);?/g, "import $2 from $4;")
		.replace(/USE\((['"][.\w/]+['"])\);?/g, "import $1;")
