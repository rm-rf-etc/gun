...
export default function match(t, o){
	let result = false;
	t = t || '';
	...
	if(Type.obj.has(o,'*')){ if(t.slice(0, o['*'].length) === o['*']){ result = true; t = t.slice(o['*'].length) } else { return false }}
	if(Type.obj.has(o,'!')){ if(t.slice(-o['!'].length) === o['!']){ result = true } else { return false }}
	if(Type.obj.has(o,'+')){
		if(Type.list.map(Type.list.is(o['+'])? o['+'] : [o['+']], function(m){
			if(t.indexOf(m) >= 0){ result = true } else { return true }
		})){ return false }
	}
	if(Type.obj.has(o,'-')){
		if(Type.list.map(Type.list.is(o['-'])? o['-'] : [o['-']], function(m){
	...
	if(Type.obj.has(o,'>')){ if(t > o['>']){ result = true } else { return false }}
	if(Type.obj.has(o,'<')){ if(t < o['<']){ result = true } else { return false }}
	...
	if(Type.obj.has(o,'?')){ if(fuzzy(t, o['?'])){ result = true } else { return false }} // change name!
	return result;
};
