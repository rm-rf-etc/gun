module.exports = (Gun) => {
	...
	s3.chain.PUT = function(key, o, cb, m){
		...
		m.Key = m.Key || key;
		if(Gun.obj.is(o) || Gun.list.is(o)){
			m.Body = Gun.text.ify(o);
			m.ContentType = 'application/json';
		} else {
			m.Body = Gun.text.is(o)? o : Gun.text.ify(o);
		}
	...
	s3.chain.dbs = function(o, cb){
		...
		this.S3().listBuckets(m, function(e,r){
			//Gun.log('dbs',e);
			Gun.list.map((r||{}).Contents, function(v){console.log(v);});
			//Gun.log('---end list---');
			if(!Gun.fn.is(cb)) return;
			cb(e,r);
		});
